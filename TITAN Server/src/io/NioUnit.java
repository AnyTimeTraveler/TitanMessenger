...
public class NioUnit implements Runnable {
	 ...
	 * @throws IOException
	 */
	public NioUnit(int port, EventHandler eventHandler) throws IOException {
		this.hostPort = port;
		this.eventHandler = eventHandler;
		this.selector = this.initSelector();
	 ...
	 */
	public SocketChannel connect(InetAddress remoteHost, int remotePort, int timeout) throws IOException, InterruptedException {
	...
	public void setConnectionInfo(SocketChannel channel, ConnectionInfo info) {
		synchronized (this.pendingChanges) {
	...
	public ConnectionInfo getConnectionInfo(SocketChannel socketChannel) {
		return (ConnectionInfo) socketChannel.keyFor(selector).attachment();
	}
	...
	private void read(SelectionKey key) throws IOException {
		...
		eventHandler.handleEvent(new DataReceivedEvent(socketChannel, recBytes));
	}
	...
	private void createServerSocketChannel() throws IOException, ClosedChannelException {
		...
		InetSocketAddress isa = new InetSocketAddress(this.hostPort);
		serverChannel.socket().bind(isa);

		eventHandler.handleEvent(new LogEvent("Now listening on port: " + hostPort));
